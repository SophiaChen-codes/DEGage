...
#'@export
DEGage <- function(counts, group, perm.preprocess = FALSE,
  ...
  outputdf$pval <- pdf_facilitator(outputdf, ncores, cl)

  #Performs ZINB regression, then recalculates P values for genes with NA's
  outputdf <- run.ZINB.refitting(counts, group, outputdf, cl, ncores)
  rm(counts) ##free up some memory early
...
#'#'@param min_nFeatureRNA The minimum number of genes that must be present in a cell
#'@param max_nFeatureRNA The maxiumum number of genes that can be present in a cell
...
DEGage_preprocess <- function(input, dir.type = 'mtx', min.nFeatureRNA = 200,
  ...
  init.preprocess.dependencies()
  sobj <- parse.input(input, dir.type)
...
DEGage_complete <- function(input, dir.type = 'mtx',
                            ...
                            gene.filter.threshold = 1, min.nFeatureRNA = 200,
                            max.nFeatureRNA = 8000, mt.percent = .2,
                            cell.annotations = TRUE,
