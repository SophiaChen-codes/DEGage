...
gen_rownames <- function(ngenes, ndegs, ncores){
  ...
  Degnames <- parSapply(cl = cl, X = c(1:ndegs), FUN = function(x) paste("DEG",x, sep = ""))
  nondegnames <- parSapply(cl = cl, X = c((ndegs+1): ngenes), FUN = function(x) paste("GENE",x, sep = "" ))
...
gen_colnames <- function(ncells, ngroup1, ncores){
  ...
  group1.titles <- parSapply(cl = cl, X = 1:ngroup1, FUN = function(x)paste("Cell",x,".Group1", sep = "" ))
  group2.titles <- parSapply(cl = cl, X = (ngroup1+1):ncells, FUN = function(x)paste("Cell",x,".Group2", sep = "" ))
...
DEGage_Simulation <- function(ngenes, ndegs, cellgroups, lfc = 1, prop.zeros = .3, seed = NULL, ncores = 4){
  ...
  if(!suppressPackageStartupMessages(require("parallel"))){
    #used for parallelizing apply family funtions
  ...
  DEGgroup1list <- parLapply(cl = cl, X = DEGmeans, fun = function(x)rnbinom(n = ngroup1, size = size.list, mu = x))
  ...
  DEGgroup2list <- parLapply(cl = cl, Group2means.adj, fun = function(x) rnbinom(n = ngroup2, size = rgamma(1,1), mu = x))
  ...
  nondeglist <- parLapply(cl = cl, X = mean.list[(ndegs+1):length(mean.list)], fun = function(x) rnbinom(n = ncells, size =  runif(1, min = .01, max = 2), mu = x))
  ...
  finaldf <- parApply(cl=cl, X = finaldf, MARGIN = 1, FUN = introduce_dropouts, ncells = ncells, ndropouts = ndropouts)
